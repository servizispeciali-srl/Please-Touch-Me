    ...
    implements Cloneable
{
    ...
    private void readImpl(InputStream in, int offset, int length)
        throws IOException
    {
        int readcount;
    ...
    public long popU4B()
    {
        return (long)(pop() & 0xff) << 24 | (long)(pop() & 0xff) << 16 | (long)(pop() & 0xff) << 8 | (long)(pop() & 0xff);
    }
    ...
    public void write(OutputStream out, int length)
        throws IOException
    {
        if(length == 0)
            return;
    ...
    public byte peek(int index)
    {
        if(index >= size)
        {
            throw new IllegalArgumentException((new StringBuilder()).append("index ").append(index).append(" is >= queue size ").append(size).toString());
        } else
        {
    ...
    public int indexOf(byte b[], int start)
    {
        ...
        for(; (start = indexOf(b[0], start)) != -1 && start < (size - b.length) + 1; start++)
        {
            boolean found = true;
            int i = 1;
            do
